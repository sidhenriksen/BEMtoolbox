    ...
    methods
        ...
        function rds = RDS(varargin)
            ...
            for j = 1:2:length(varargin);
                assert(isprop(rds,varargin{j}),'Property not in generator structure');
        ...
        function [L,R] = generate(rds)
            ...
            % Indices for uncorrelated dots
            uc_indices = (n_dots-n_uncorrelated+1):n_dots;
            ...
            R_centers = L_centers; 
            R_centers(uc_indices,:) = ...
                [randi(m_Ny-(rds.dotsize-1),n_uncorrelated,1),...
                randi(m_Nx-(rds.dotsize-1),n_uncorrelated,1)];

            
            
            % If we want to this randomly instead, we can do something like
            % this; and do something similar to correlation
            fill_L = (rand(1,n_dots)<rds.polarity)*2 -1;
            
            fill_R = fill_L;
            fill_R(uc_indices) = (rand(1,n_uncorrelated) < rds.polarity)*2 -1;
